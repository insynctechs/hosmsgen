...
{
    class Investigations : IDisposable
    {
        string conn = HospitalERP.Helpers.DBHelper.Constr;
       
        public int InsertInvestigations(string name, string desc, int type, decimal fee, bool active)
        {
            ...
            Int32 Id = Convert.ToInt32(SqlHelper.ExecuteScalar(conn, CommandType.StoredProcedure, "uspInvestigations_Add", sqlParam).ToString());
            return Id;
        }
        ...
        public DataTable GetRecords(string SearchBy, string SearchValue)
        {
            try
            {
                ...
                DataSet dt = SqlHelper.ExecuteDataset(conn, CommandType.StoredProcedure, "uspInvestigations_Get", sqlParam);
                return dt.Tables[0];
            }
            catch (Exception ex)
            {
        ...
        public int editInvestigations(int id, string name, string desc, int type, decimal fee, bool active)
        {
            int ret = -1;
            try
            {
                ...
                ret = Convert.ToInt32(SqlHelper.ExecuteScalar(conn, CommandType.StoredProcedure, "uspInvestigations_Edit", sqlParam).ToString());
            }
            catch (Exception ex)
            {
                ret = -1;
        ...
        public DataTable getRecordFromID(int id)
        {
            try
            {
                ...
                sqlParam[0] = new SqlParameter("@id", id);
                DataSet dt = SqlHelper.ExecuteDataset(conn, CommandType.StoredProcedure, "uspInvestigations_Single", sqlParam);
                return dt.Tables[0];
            }
            catch (Exception ex)
            {
