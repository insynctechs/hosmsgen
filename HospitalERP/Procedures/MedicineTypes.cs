...
{
    class MedicineTypes : IDisposable
    {
        ...
        public int InsertProc(string DepartmentName,string description, bool Active)
        {
            ...
            Int32 Id = Convert.ToInt32(SqlHelper.ExecuteScalar(conn, CommandType.StoredProcedure, "uspMedicineTypes_Add", sqlParam).ToString());
            return Id;
        }
        ...
        public DataTable GetRecords(string SearchBy, string SearchValue)
        {
            try
            {
                ...
                DataSet dt = SqlHelper.ExecuteDataset(conn, CommandType.StoredProcedure, "uspMedicineTypes_Get", sqlParam);
                return dt.Tables[0];
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetProcTypesIDName(int id)
        {
            try
            {
                ...
                DataSet dt = SqlHelper.ExecuteDataset(conn, CommandType.StoredProcedure, "uspMedicineTypes_Combo", sqlParam);
                return dt.Tables[0];
            }
            catch (Exception ex)
            {
        ...
        public int editTypes(int id, string name, string desc, bool active)
        {
            ...
            try
            {
                ...
                ret = Convert.ToInt32(SqlHelper.ExecuteScalar(conn, CommandType.StoredProcedure, "uspMedicineTypes_Edit", sqlParam).ToString());
            }
            catch (Exception ex)
            {
                ret = -1;
