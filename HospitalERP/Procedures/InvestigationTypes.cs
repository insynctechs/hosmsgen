...
{
    class InvestigationTypes : IDisposable
    {
        string conn = HospitalERP.Helpers.DBHelper.Constr;
       
        
        public int InsertInvesType(string DepartmentName,string description, bool Active)
        {
            ...
            Int32 Id = Convert.ToInt32(SqlHelper.ExecuteScalar(conn, CommandType.StoredProcedure, "uspInvestigationTypes_Add", sqlParam).ToString());
            return Id;
        }
        ...
        public DataTable GetRecords(string SearchBy, string SearchValue)
        {
            try
            {
                ...
                DataSet dt = SqlHelper.ExecuteDataset(conn, CommandType.StoredProcedure, "uspInvestigationTypes_Get", sqlParam);
                return dt.Tables[0];
            }
            catch (Exception ex)
            {
        ...
        public DataTable GetInvesTypesIDName(int id)
        {
            try
            {
                SqlParameter[] sqlParam = new SqlParameter[1];
                sqlParam[0] = new SqlParameter("@id", id);
                DataSet dt = SqlHelper.ExecuteDataset(conn, CommandType.StoredProcedure, "uspInvestigationTypes_Combo", sqlParam);
                return dt.Tables[0];
            }
            catch (Exception ex)
            {
        ...
        public int editInvesTypes(int id, string name, string desc, bool active)
        {
            int ret = -1;
            try
            {
                ...
                ret = Convert.ToInt32(SqlHelper.ExecuteScalar(conn, CommandType.StoredProcedure, "uspInvestigationTypes_Edit", sqlParam).ToString());
            }
            catch (Exception ex)
            {
                ret = -1;
