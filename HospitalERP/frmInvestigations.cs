...
namespace HospitalERP
{
    public partial class frmInvestigations : Form
    {
        ...
        Investigations objInves = new Investigations();
        ...
        public frmInvestigations(int width, int height)
        {
            InitializeComponent();
        ...
        private void PopulateSearch()
        {
            try
            {
                cmbSearch.DataSource = objInves.SearchValues();
        ...
        private void PopulateProcTypeCombo(int tid)
        {
            try
            {
                using (InvestigationTypes insType = new InvestigationTypes())
                {
                    cmbProcType.DataSource = insType.ProcTypesCombo(tid);
        ...
        private void ShowProcedureTests()
        {
            try
            {
                DataTable dtRecords = objInves.GetRecords(cmbSearch.SelectedValue.ToString(), txtSearch.Text);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidateChildren(ValidationConstraints.Enabled))
                {
                    ...
                    {
                        rtn = objInves.InsertInvestigations(txtName.Text, txtDesc.Text, Convert.ToInt32(cmbProcType.SelectedValue.ToString()), Convert.ToDecimal(txtFee.Text), chkActive.Checked);
                        if (rtn == -1)
                        {
                    ...
                    {
                        rtn = objInves.editInvestigations(Int32.Parse(txtID.Text.Trim()), txtName.Text, txtDesc.Text, Convert.ToInt32(cmbProcType.SelectedValue.ToString()), Convert.ToDecimal(txtFee.Text), chkActive.Checked);
                        if (rtn == 0)
        ...
        private void txtName_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                /*else if (objInves.GetRecords("p.name", txtName.Text.Trim()).Rows.Count > 0)
                {
                    e.Cancel = true;
                    //txtName.Focus();
        ...
        private void setFormFields(int index)
        {
            try
            {
                ...
                DataTable dt = objInves.getRecordFromID(Convert.ToInt32(txtID.Text));
        ...
        private void frmInvestigations_Shown(object sender, EventArgs e)
        {
            try
            {
                PopulateSearch();
        ...
        private void frmInvestigations_FormClosed(object sender, FormClosedEventArgs e)
        {
            try
            {
                if (this.IsMdiChild)
